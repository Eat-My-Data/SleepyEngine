...
float4 main(float3 viewFragPos : Position, float3 viewNormal : Normal, float3 viewTan : Tangent, float3 viewBitan : Bitangent, float2 tc : Texcoord, float4 lightViewPos : Position2) : SV_Target
{
    ...
    specularPower = directionalLightData[0].specularPower;
    ...
    for (float i = 0; i < 2; i++)
    {
	    ...
	    // specular
        combinedPointLightSpecular += Speculate(specularColor, pointLightData[i].diffuseIntensity, viewNormal, lv.vToL, viewFragPos, att, specularPower);
    }
